...
import { getFactor } from '../wholesUtil';
...
class EditPart extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.reRender=='init'||nextProps.reRender==undefined){
    this.setState({
  ...
  shouldComponentUpdate(nextProps, nextState){
    ...
    if(nextProps.reRender=='noRender'&&nextProps.reRender=='shuldRender'){
      return false
    }
      ...
      <div>
        {
          newEditItemList&&newEditItemList.length>0? newEditItemList.map((editItem, idx) => {
            ...
            return (
              ...
              >
                <Row className={pageSetting&&pageSetting.wordBreak? '' :'flex-nowrap'}>
                  <Col span={JSON.stringify(contStyle)!=='{}' ? 8 : ''} style={Object.assign({}, styles.text, textContStyle, pageSetting&&pageSetting.wordBreak? {} :{'textAlign': 'right'}) }  className="text-align-right maxwidthx-130 text-overflow padding-right-3" title={editItem.text}>
                  {
        ...
        <Row justify={'flex-start'} className=" display-inline-block">
          <ThemeContext.Consumer>
            ...
            callback={()=>{that.props.callback()}}   doParentAction={()=>{
              that.save((res, callback)=>{
                // sessions.setStorage(`${that.props.pageData.url}-forData`, res)
                context.parentCall(res, that.props.pageData, callback)
              })  
            }} doParentSubmitAction={(btn, resolve, reject, callback)=>{
              that.save((res)=>{
                context.parentSubmitCall(res, btn, resolve, reject, callback)
              })
            }}/>)
          }}
          </ThemeContext.Consumer>
