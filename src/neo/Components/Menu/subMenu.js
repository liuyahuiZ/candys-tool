...
class SubMenu extends Component {
  ...
  setShow() {
    this.setState({ display: 'show', iconName: 'ios-arrow-up' });
  }
  setHide() {
    this.setState({ display: 'hide', iconName: 'ios-arrow-down' });
  }
  // 正常模式下
  toggle() {
  ...
  render() {
    const { subUlBg, fontDefalutColor, fontActiveColor,
      ...
      subUlBorder, BoxShadow, borderRight } = theme[this.props.themes];
    ...
    const borderStyle = this.props.active ? {'borderRight': borderRight} : '';
    ...
    // preIcon
    const preIcon = (<Icon iconName={this.props.iconName} iconColor={IconColor} size={'150%'} />);
    let preIconItem = (<span style={styles.preIcon} >{preIcon}</span>);

    // 层级处理
    const level = this.props.level;
    const childPadding = level > 1 ? { paddingLeft: `${level * 10}px` } : '';
    ...
    let title = (<span
      ...
      data-content={JSON.stringify(self.props.item)}
    >
      {this.props.title}
    </span>);
    // 路由
    ...
    if (Children) {
      if (Children.length === 0) {
        item = (<div
          style={arrayUtils.merge([styles.painner, activedStyle, childPadding, borderStyle ])}
          onMouseLeave={self.handleMouseLeave}
          onMouseEnter={self.handleMouseEnter}
          title={self.props.item.describe || self.props.item.title}
          data-content={JSON.stringify(self.props.item)}
        >
          {preIconItem}
          {title}
        ...
        >
          ...
          <span style={styles.switch}>
            <Icon style={arrayUtils.merge([styles.menuSwitch, hoverStyle])} title={self.state.display =="show" ? '收拢菜单' : '展开菜单'} iconName={self.state.iconName} iconColor={self.state.display =="show" ? iconActiveColor : iconDefalutColor || IconColor} size={'110%'} />
          </span>
        </div>);
