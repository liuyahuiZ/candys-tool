...
class PopOver extends Component {
  ...
  componentDidMount() {
    ...
    document.addEventListener('click', (e) => {
      ...
      const child = self.$$popover&&self.$$popover.children;
      const contentchild = self.$$popcontent&&self.$$popcontent.children;
      if(!child) return;
      if(!contentchild) return;
      let status = true;
      let num = '';
      for (let i = 0; i < child.length; i++) {
        if (e.target === child[i]||e.target.parentNode === child[i]) {
          status = false;
          num = i;
        }
      }
  ...
  render() {
    const { popContent, placement, popStyle } = this.props;
    ...
    return (
      ...
      >
        ...
        <Transition
          ...
          leave={'pop-leave'}
        ><div
          style={arrayUtils.merge([styles.popoverContent, popoverStyle, placementStyle, popStyle])}
