...
import SearchListPageView from '../modals/searchListPageView';
...
class SearchListPage extends Component {
    ...
    render() {
        ...
        { tabName: '配置listTabel', keyword: '6', content: (
          <Row className="bg-show padding-all font-size-9">
            ...
            <Col span="16" className="padding-top-1r"> <Switch value={listConfig&&listConfig.operationsFix || false}  checkedText={'-'} unCheckText={'o'} onchange={(data)=>{ 
                            self.setKey('operationsFix', data);
                        }} /> { listConfig&&listConfig.operationsFix ? '固定': '不固定'}
            </Col>
          </Row>
        ) },
          { tabName: '编辑结果字段', keyword: '3', content: (<EditList datas={listFormat} handelChange={(res)=>{
            this.setState({
              listFormat: res
            })
