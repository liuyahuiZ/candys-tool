...
class DetailPageView extends Component {
    ...
    render() {
        const {detailCondition, pageParmes, detailUrlCondition, detailVideoCondition} = this.state
        let pageProps = {
          itmsConfig: detailCondition,
          cardBgConfig: detailUrlCondition,
          videoConfig: detailVideoCondition,
        ...
        return(
          <>
            <ThemeContext.Consumer>
                { context => { 
                  return (
                <VideoPic {...pageProps} reRender={context.reRender} contextParames={context.parames} ref={(r)=>{this.$$DetailView = r}} />)
                }
              }
              </ThemeContext.Consumer>
          </>
